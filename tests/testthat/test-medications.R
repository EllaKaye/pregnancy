...
test_that("defaults spanning full date range", {
  meds <- readRDS(test_path("fixtures", "meds.rds"))
  summary <- medications_remaining(
    meds,
    on_date = as.Date("2025-03-31"),
    until_date = as.Date("2025-04-10")
  )
  expect_equal(summary$medication, c("A", "B", "C"))
  expect_equal(summary$quantity, c(4, 6, 3))
})
...
test_that("earlier until_date", {
  ...
  summary <- medications_remaining(
    meds,
    on_date = as.Date("2025-03-31"),
    until_date = as.Date("2025-04-03")
  )
  expect_equal(summary$medication, c("A", "B"))
  expect_equal(summary$quantity, c(3, 2))
})
...
test_that("later on_date", {
  ...
  expect_equal(summary$quantity, c(2, 3))
})
...
test_that("group by format", {
  ...
  expect_equal(summary$quantity, c(3, 10))
})
...
test_that("date order matters", {
  ...
  expect_error(
    medications_remaining(
      meds,
      until_date = as.Date("2025-03-31"),
      on_date = as.Date("2025-04-10")
    ),
