...
To easily keep this vignette fairly up-to-date, 
and prevent recurring package build and CRAN failures as time goes by, 
I have not hard-coded any dates here. 
Instead, I have calculated everything from the date on which this vignette built. 
That was on **`r format(Sys.Date(), '%B %d, %Y')`**, 
...
It defaults to providing this information for the current date, but an alternative `on_date` can be provided. 
See the section below on global options for how to avoid needing to pass in the `due_date` value every time you call these function.
